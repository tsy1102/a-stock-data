...
### 🆕 新增数据源封装（sc_datasource.py，带缓存+限流）

| 封装 | 字典依据 | 实测 |
|:---|:---|:---|
| `get_cls_market_emotion` | §12.10.2 财联社市场情绪 | ✅ 热度62/封板率90%/炸板15/高开率88%/连板梯队 |
| `get_kph_limit_ladder` | §12.10.4 开盘红涨停天梯 | ✅ 75 只（传智教育6板/大单一字/人气）|
| `get_stock_changes` | §12.10.1 东财盘口异动 | ✅ 2792 只火箭发射 |
| `get_shortline_indicators` | §12.12.1 AxData 短线指标 34 字段 | ✅ 茅台开盘量比1.01/竞价昨比0.58/自由流通540949000股 |

### 🔧 脚本接入

1. **mak 报告**：A 章节接入财联社市场情绪（热度/封板率/炸板/高开率/获利率/连板梯队）；新增 B+ 章节（开盘红涨停天梯 + 东财盘口异动）
2. **sht 报告**：打板分析新增 4 类盘口异动个股检测（火箭发射/大笔买入/封涨停板/有大买盘）+ AxData 短线指标展示
3. **is_limit_up/is_limit_down 官方规则修正**（sc_utils.py）：ST 9.5%→**5%**、北交所 30%（老号段 8/4 + 新号段 92）——按字典 §12.12.3 limit_rule 枚举；补北交所测试用例
4. **requirements.txt**：新增 levistock/axdata（可选依赖，缺失时自动降级）

### ✅ 验证

- 262 测试全过（+1 北交所用例）
- 新数据源 4/4 实测可用（网络 + 依赖齐全时）
