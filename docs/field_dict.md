...
| **[12]** | `unseal_date` | ✅ **= 新股开板日 (YYYYMMDD)** | ❌→✅ | 日期 | `""` / `""` | **V16.2.18 破解**（东财 f189 交叉）：2016+ 新股上市后首次不再涨停的日期；与 f189 上市日差值=连板交易日数（001203 大中矿业 10 日历日=8 交易日✓、300750 宁德 8 交易日✓、24 样本 18/24 精确、余差 1 天为节假日近似）。老股/2015 前为空 |
| **[13]** | `board_count` | ✅ **= 上市连板数（交易日）** | ❌→✅ | 整数 | `""` / `""` | **V16.2.18 破解**：开板日-上市日间的交易日数（18/24 精确匹配；300750=8 连板✓、001223 首日开板=0✓）。与 Col[12] 构成"次新股开板"数据对 |
...
| **[17]** | `change_20d` | ✅ **近20根K线涨跌幅** | ❌→✅ | `float` | `10.55` / `8.77` | **V16.2.18 修正**（injoyai 130 日日线核验 MAE0.23）：原误标"20日"，实为"近20根K线"（交易日口径，含当日）。**注：zhb_client 的 change_20d key 现映射 Col[18]** |
| **[18]** | `change_30d` | ✅ **= 20日涨跌幅 (%)** | ❌→✅ | `float` | `8.50` / `7.91` | **V16.2.18 修正**（injoyai 核验 MAE0.23）：原误标"30日"（不存在 30 日字段）——**key 名 change_30d 为历史遗留，实为 20 日** |
| **[19]** | `change_60d` | ✅ **= 近60根K线涨跌幅** | ❌→✅ | `float` | `-0.45` / `-6.09` | **V16.2.18 修正**（injoyai 核验 MAE≈0）：原误标"60日"，实为"近60根K线"（交易日口径）。zhb_client 已另设 change_60k_bar 精确名 |
| **[20]** | `change_60d_alt` | ✅ **= 60日涨跌幅 (%)** | ❌→✅ | `float` | `0.09` / `-6.35` | **V16.2.18 修正**（injoyai 核验 MAE≈0）：原"疑似90日"错误，实为 60 日涨跌幅（日历日口径）|
...
| **[4]** | *(丢弃)* | ✅ **疑=涨跌停封单额 (万元)** | ⚠️ | `float` | `""` / `""` | **V16.2.18**：0804 有值 144 只中 143 只涨跌停(99%)，600530 跌停 -10.02% 也有值；122 只涨停无值(未封住/封单 0)。值如 000533=9207.45 / 000593=14272.09 |
| **[5]** | `amount_1d` | **T-1 日总成交额** | ✅ | **万元** | — / `462224.28` | 与 Col3 形成滚动时序 |
| **[6]** | *(丢弃)* | 疑=资金分档(大单类) | ⚠️ | `float` | `""` / `""` | **V16.2.18**：有值 93 只,涨停占比 26%；000533 涨停=10948.26 > 主力净买 6249.73(分档口径) |
| **[7]** | `amount_2d` | **T-2 日总成交额** | ✅ | **万元** | — / `439250.53` | T-2日成交额 |
| **[8]** | *(丢弃)* | 疑=资金分档(特大单类) | ⚠️ | `float` | `""` / `""` | **V16.2.18**：有值 100 只,涨停占比 28%；000009=1063.63 |
| **[9]** | `main_net_buy_hands` | **T 日主力净买量** | ✅ | **手** | `4667` / `1366` | ⭐⭐⭐⭐⭐ 100%验算破译 |
| **[10]** | `main_net_buy_hands_1d` | **T-1 日主力净买量** | ✅ | **手** | — / `757` | 主力资金手数量时序对 |
| **[11]** | *(丢弃)* | ⚠️ **5日涨跌幅变体(非复制)** | ⚠️ | `float` | — / `8.77` | V16.3 D2：与 tdxstat Col[27] change_5k_bar **±0.5 内 21%**（corr 0.75），非复制（基准/复权差异）；**原"与 Col17 完全相同"判断错误(corr 0.03)** |
| **[12]** | *(丢弃)* | ⚠️ **长周期涨跌幅变体** | ⚠️ | `float` | — / `-8.09` | V16.3 D2：与 tdxstat Col[21] change_ytd 最接近（精确 408/7953、±0.5 内 652）；非复制；**非配股比例**（文档 Part2 张冠李戴，配股比例在 tipinfo Col[12]）|
| **[13]** | `industry_code` | **T 日特色板块归属** ⚠️ 非行业 | ❌ | 6位字符串 | — / `880823` | **V16.2.17 重新定性**：通达信 T 日动态条件板块（微盘股/近已解禁/业绩预降/一带一路…），成员按当日条件筛选（880823 微盘股 197 只成交额全部≤2.93 亿验证），个股只取其一 → 每日变化、不全。**不可用于行业聚合**；行业请用 881 申万段（`is_industry_code`）或东财申万二级映射 |
...
| **[19]** | *(丢弃)* | ⚠️ **20日涨跌幅变体(非复制)** | ⚠️ | `float` | — / `3.73` | V16.3 D2：与 tdxstat Col[18] change_20d **±0.5 内 9%**（corr 0.88），非复制；疑后复权/基准差异；**非配股价**（文档 Part2 张冠李戴，配股价在 tipinfo Col[20]）|
| **[20]** | *(丢弃)* | ⚠️ **涨跌幅变体(20d/60k 间)** | ⚠️ | `float` | — / `2.03` | V16.3 D2：与 20d/20k/60k 均接近（精确 ~130/7953）；非复制 |
...
| **[19]** | *(丢弃)* | **(新/最近)增发事件日期** | ⚠️→✅ | `""` | V16.3 D2：000100=20260603、600519=20251106 |
