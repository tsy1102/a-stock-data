#!/usr/bin/env python3
"""
get_val_report.py — 18 策略全市场发现引擎 (V8)
...
def strategy_05_double_bottom(stocks, top_n=500):
    ...
    for s in candidates:
        ...
        if idx_vol >= 0:
            ...
            if len(vols) >= 5:
                vol_increasing = vols[-1] > vols[-3] * 1.2
            else:
                vol_increasing = True
        else:
            vol_increasing = True
...
def run_discovery(output_path):
    ...
    L("=" * 85)
    L(f"  🔍 全市场18策略发现引擎V8 — {today_str} {_t_now.strftime('%H:%M:%S')}（{_mkt_note}）" )
    L("=" * 85)
...
async def run_discovery_async(output_path):
    ...
    L("=" * 85)
    L(f"  A 股策略发现报告 V8  [{today_str} {_t_now.strftime('%H:%M:%S')}]")
    L("=" * 85)
...
if __name__ == "__main__":
    args = common_parse_args("18 策略全市场发现引擎V8")
    base_dir = os.path.dirname(os.path.abspath(__file__))
    ts = datetime.now().strftime("%Y%m%d_%H%M")
    op = os.path.join(args.output, f"get_val_report_{ts}.txt")
    print(f"🚀 全市场18策略发现引擎V8启动 — {date.today()}", flush=True)
