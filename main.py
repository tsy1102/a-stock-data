#!/usr/bin/env python3
"""main.py — V8 统一 CLI 入口（脚本内 asyncio 并发 + 进程级串行防封）
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="V8: A股数据工具 — 统一入口（并发版，防封限流）",
...
async def main_async():
    ...
    print(f"[{ts}] V8 批量报告启动 | 并发度: {conc} | 输出目录: {output_dir}", flush=True)
    ...
    # 汇总
    print(f"\n{'=' * 60}", flush=True)
    print(f"[完成] V8 批量报告总耗时: {total_time:.1f} 秒 " +
