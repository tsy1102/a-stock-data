...
def _patch_easy_tdx_heartbeat() -> None:
    try:
        ...
        def _safe_heartbeat_loop(self, *args, **kwargs) -> None:
...
def _check_tdx() -> bool:
    ...
    with _TDX_CALL_LOCK:
        ...
        try:
            ...
            c = TdxClient()
            c.connect()
            quotes = c.get_security_quotes([(1, "600519")])
...
def tdx_get_historical_high(code: str) -> Optional[float]:
    ...
    with _TDX_CALL_LOCK:
        ...
        try:
            ...
            return max(b.high for b in bars if b.high > 0)
...
def tdx_get_belong_boards(code: str):
    with _TDX_CALL_LOCK:
        ...
        try:
            ...
            if df is None or df.empty: return {}
            result = {"industry": [], "concept": [], "area": [], "style": []}
